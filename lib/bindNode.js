...
const bindNode = (node, type, component, parsed, extra) => {
    ...
    bindNode.currentWatcherStack.unshift(newWatcher);

    if (closestArrayWatcher){
        bindNode.closestArrayWatcherStakc.unshift(closestArrayWatcher);
    } 

    triggerWatcher(newWatcher);

    bindNode.currentWatcherStack.shift();
    if (closestArrayWatcher){
        bindNode.closestArrayWatcherStakc.shift(closestArrayWatcher);
    }
};

bindNode.currentWatcherStack = [];
bindNode.closestArrayWatcherStakc = [];
